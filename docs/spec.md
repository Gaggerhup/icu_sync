...
8. `icuconsult_api_lib.py` แปลง schema payload ภายในเป็น external API payload ตามเอกสาร `external-client-icucon-api.md` ก่อนส่งจริง
9. เว็บ ICUCONS ตรวจและเติม schema ที่ยังขาดก่อน insert เพื่อกัน error `Unknown column ... in INSERT INTO`
10. เว็บ ICUCONS สร้างหรืออัปเดตข้อมูลลงตาราง `app_patient`, `patient_allergy`, `app_patient_condition`, `case_register`, `case_vital`, `case_lab`, `case_medication`, และ `case_note`
11. `Main_logic.py` เก็บ payload ล่าสุดไว้ที่ `self.schema_payloads`
12. UI แสดง summary จำนวน case, vital, lab, medication และ case id ที่เว็บคืนกลับมา
